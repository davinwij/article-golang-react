...
function Draft() {
  ...
  const checkArticleStatus = async () => {
    ...
    localStorage.setItem("draftLength", draftArticle.length)
  }
  ...
  return (
    <>
      <CRow>
        <NavTabs draftLength={draftArticles.length} />
      </CRow>
      <CRow>
        <CTable striped>
          <CTableHead>
          ...
          <CTableBody>
            {draftArticles.map((item, index) => (
              <CTableRow key={item.id}>
                ...
                <CTableDataCell>{item.title} <span style={{ color: "red"}}>(draft)</span></CTableDataCell>
